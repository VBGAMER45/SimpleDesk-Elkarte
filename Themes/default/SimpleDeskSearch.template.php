<?php
/**
 * SimpleDesk Search Template
 *
 * Templates for helpdesk search functionality including the search form,
 * results display, criteria summary, and page navigation.
 *
 * @package SimpleDesk
 * @version 2.1.5
 */
...
/**
 * Displays the no-results page after a search.
 *
 * @since 2.0
 */
...
/**
 * Displays the search criteria used in the last search.
 *
 * @since 2.0
 */
function template_search_criteria()
{
	...
	<div class="information">
		...
		{
			echo '
			<li>', $txt['shd_search_text'], ' ', Util::htmlspecialchars($context['search_terms']), ' (', $context['match_all'] ? $txt['shd_search_match_all'] : $txt['shd_search_match_any'], ')</li>';
		...
		<div class="windowbg core_posts">
			<div class="content flow_auto">
				<div class="topic_details floatleft">
					...
					<h5>', $result['dept_link'], '<a href="', $scripturl, '?action=helpdesk;sa=ticket;ticket=', $result['id_ticket'], '">', sprintf($result['is_ticket'] ? $txt['shd_search_result_ticket'] : $txt['shd_search_result_reply'], $result['display_id']), '</a> - ', $result['subject'], ' (', $txt['shd_search_last_updated'], ' ', standardTime($result['last_updated']), ')</h5>
					<span class="smalltext">&#171;&nbsp;<strong>', $result['is_ticket'] ? $txt['shd_search_ticket_opened_by'] : $txt['shd_search_ticket_replied_by'], ' ', shd_profile_link($result['poster_name'], $result['id_member']), '</strong>&nbsp;', $txt['on'], '&nbsp;<em>', standardTime($result['poster_time']), '</em>&nbsp;&#187;</span>
				</div>
				<br>
...
/**
 * Renders a page navigation button via POST form.
 *
 * Uses form POST rather than links to avoid server hammering from
 * search re-execution on every page navigation click.
 *
 * @param int|string $page Page number, 'prev', or 'next'.
 * @since 2.0
 */
function template_search_navigation($page = 'next')
{
	...
	// Search terms.
	echo '
		<input type="hidden" name="search" value="', Util::htmlspecialchars($context['search_terms']), '">';
...
/**
 * Javascript for Search autosuggest fields.
 *
 * Initializes elk_AutoSuggest for ticket starter and assignee name fields.
 *
 * @since 2.1
 */
function template_shd_js_search()
{
	...
	echo '
				<script>
					var oTicketStarter = new elk_AutoSuggest({
					...
					var oTicketAssignee = new elk_AutoSuggest({
