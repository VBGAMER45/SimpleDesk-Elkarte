<?php
/**
 * SimpleDesk Profile Template
 *
 * Templates for helpdesk user profile pages including:
 * - Profile main/home page with ticket stats
 * - Preferences management
 * - Show tickets/replies listing
 * - Permissions view per department
 * - Action log display
 * - Monitor/ignore notification overrides
 * - Profile navigation sidebar
 *
 * Ported from SMF SimpleDesk-Profile.template.php to ElkArte.
 *
 * @package SimpleDesk
 * @version 2.1.5
 */
...
/**
 * Display the profile home/main section.
 *
 * Shows ticket statistics (total, open, assigned), links to
 * view tickets, create new ticket, and post proxy. In helpdesk-only
 * mode, also displays basic account information.
 *
 * @since 2.0
 */
function template_shd_profile_main()
{
	...
	if (!empty($context['can_post_proxy']))
		echo '
					<a href="', $scripturl, '?action=helpdesk;sa=newticket;proxy=', $context['member']['id'], '" class="button"><span id="shd_post_proxy"></span>', $txt['shd_profile_proxy'], '</a>';
	...
	// In helpdesk-only mode, we don't have the forum profile, so we need to display what's useful and relevant.
 ...
 *
 * Shows grouped preferences (display, blocks, notifications) with
 * appropriate input types (checkbox, select, int) for each preference.
 * Includes collapsible sections and a save button.
 *
 * @since 2.0
 */
...
/**
 * Display the show tickets/replies page.
 *
 * Shows a paginated list of tickets or replies for the profile member.
 * Includes navigation tabs (Tickets, Replies, Monitor, Ignore),
 * and for each item: subject, time, body excerpt, and view link.
 *
 * @since 2.0
 */
function template_shd_profile_show_tickets()
{
	...
	{
		// Build the link for tickets vs replies
...
/**
 * Display the monitor/ignore notification override lists.
 *
 * Shows a table of tickets that the user has added to their
 * monitor or ignore lists, with ticket details.
 *
 * @since 2.0
 */
...
/**
 * Display the permissions view page.
 *
 * Shows the user's helpdesk permissions per department, including:
 * - Department selector dropdown
 * - Roles assigned through membergroup membership
 * - Granted permissions with originating roles
 * - Admin bypass notice for administrators
 *
 * @since 2.0
 */
function template_shd_profile_permissions()
{
	...
	// This user has all permissions (admin). Tidy up and bail.
	...
	// We're done if only showing dept list?
	if (!empty($context['dept_list_only']))
		return;

	// Display the roles that are attached to this user, and display the groups that make that link.
	echo '
				<div class="tborder">
					<div class="cat_bar">
						<h3 class="catbg">
	...
	// Now display their granted permissions
	if (!empty($context['member_permissions']['allowed']))
	{
		...
		// Go by what's in the master list first.
		$last_permission_cat = '';
		...
		foreach ($context['shd_permissions']['permission_list'] as $permission => $details)
		{
			...
			if (empty($icon))
				continue; // don't display if it's a denied permission or no icon

			// Check if we should display this permission
			if ($ownany)
			{
...
/**
 * Display the action log for this profile member.
 *
 * Shows a table of recent helpdesk actions performed by the user,
 * with timestamps and action descriptions. Includes a link to the
 * full admin action log if the viewer has permission.
 *
 * @since 2.0
 */
	...
	echo '
						<tr class="titlebg">
							<td class="bot_page" colspan="2">
								', !empty($context['action_full_log']) ? '<span class="smalltext shd_main_log"><img src="' . $settings['default_images_url'] . '/simpledesk/browse.png" alt=""><a href="' . $scripturl . '?action=admin;area=helpdesk;sa=actionlog">' . $txt['shd_profile_log_full'] . '</a></span>' : '', '
							</td>
						</tr>
					</table>
				</div>';
...
/**
 * Profile navigation - above layer.
 *
 * Renders the profile navigation sidebar/inline menu with links to
 * the various helpdesk profile areas (tickets, preferences, permissions, etc.).
 * Uses either sidebar or inline layout based on user theme options.
 *
 * @since 2.0
 */
...
/**
 * Profile navigation - below layer.
 *
 * Closes the wrapper div opened by template_shd_profile_navigation_above().
 *
 * @since 2.0
 */
 ...
 *
 * Wraps ElkArte's template_summary() for use in helpdesk-only mode.
 *
 * @since 2.0
 */
...
/**
 * Theme settings template for helpdesk profile.
 *
 * Adds helpdesk-specific theme options to the profile settings page,
 * including sidebar menu toggle, quick reply options, etc.
 *
 * @since 2.0
 */
...
/**
 * Javascript for Preferences collapsible sections.
 *
 * Outputs the JavaScript that manages collapsible preference groups
 * using ElkArte's elk_Toggle class (ported from SMF's smc_Toggle).
 *
 * @since 2.1
 */
function template_shd_profile_js_preferences()
{
	...
	foreach ($context['shd_preferences_options']['groups'] as $group => $details)
		echo '
		var oPreferences', $group, ' = new elk_Toggle({
			bToggleEnabled: true,
